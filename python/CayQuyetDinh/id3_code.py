...
class DecisionTreeID3(object):
    def __init__(self, max_depth= 20, min_samples_split = 2, min_gain = 1e-4):
        self.root = None
    ...
    def _split(self, node):
        ...
        for i, att in enumerate(self.attributes):
            ...
            if min(map(len, splits)) < self.min_samples_split and len(min(splits))>self.min_samples_split:
                continue
            # information gain
            HxS= 0
            for split in splits:
        ...
        return child_nodes
#entropy xác xuất điểm có thể rơi vào class
...
if __name__ == "__main__":
    df = pd.DataFrame.from_csv('bmi.csv')
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]
    print("\n",df)
    print('x=\n ',X)
